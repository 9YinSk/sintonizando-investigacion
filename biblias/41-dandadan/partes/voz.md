...
- **Su cara en cada emoción (fotograma propio, con minuto)**:
  ...
  - *Alivio/complicidad* (lo más cercano a alegría que ubiqué con fuente propia): tras sobrevivir al Flatwoods Monster, sentados juntos recuperando el aliento, ep2 (`english-sub-s-01.-e-02-11-10`) min 21:15 (*"I really thought I was gonna die!"*) y min 21:30, Momo le pregunta en broma *"What? You change your hairdo?"* — primer momento de complicidad/humor entre los dos ya sin peligro encima. ✅ (fotograma propio, minuto exacto).
  - ⚠️ No encontré un fotograma propio de risa abierta franca ni de vergüenza pura para Momo en los 4 episodios que miré (1, 2, 5-6, 7): el más cercano queda arriba (alivio/complicidad, ep2).
...
- ⚠️ Fotogramas propios de "alegría" franca (risa abierta) y "vergüenza" pura para Momo y Okarun, y de "tristeza"/"vergüenza" para Aira consciente: miré 5 episodios completos (1, 2, 5, 6, 7) más el tráiler oficial y no hubo escenas que cayeran justo en el muestreo de 15s con esas dos caras; lo más cercano queda anotado arriba con su ⚠️ (el momento de alivio/complicidad de ep2 min 21:15-21:30 es lo mejor que encontré para "alegría" de Momo/Okarun). Quedaría para una tanda extra mirar los episodios 3, 4 u 8 completos si se necesita cerrar el hueco del todo.
...
- ⚠️ Un hilo o artículo que liste explícitamente "qué le parecería falso a un fan" de Dandadan (formato exacto que sugiere ENCARGO.md): no encontré una fuente que lo diga con esas palabras; lo de la sección de punto 12 se dedujo de reseñas y datos oficiales reales, no de una lista ya armada.
...
- `fotogramas.py` sobre Internet Archive (Plan B de AYUDANTE.md, YouTube pide login desde este servidor): episodios completos `english-sub-s-01.-e-01-op-join`, `english-sub-s-01.-e-02-11-10`, `english-sub-s-01.-e-05`, `english-sub-s-01.-e-06-.compressed`, `s-01.-e-07_202411` (cada 15s, 96 fotogramas/episodio) + tráiler oficial de Crunchyroll en Dailymotion (`x96npto`, cada 2s). Todas las hojas miradas con Read. Los `video.mp4`/`audio.m4a` ya se borraron de `/tmp/claude-0/trabajo/41-dandadan-voz/` tras sacar las hojas (quedan sólo `hoja_*.jpg` e `indice.json` por episodio, ~13 MB en total, fuera del repositorio).
...
Sigue: fotograma propio de vergüenza pura para Momo/Okarun y de tristeza/vergüenza para Aira consciente (mirar episodios 3, 4 u 8 completos con `fotogramas.py`; ya cubiertos con fotograma propio: Momo rabia+tristeza+alivio, Okarun miedo+rabia+vergüenza, Turbo Granny rabia/malicia, Aira rabia+orgullo+miedo); cumpleaños/altura exactos de los 4 si aparecen en el Daizukan físico.
