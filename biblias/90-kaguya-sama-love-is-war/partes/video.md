...
- Búsqueda de la lista completa del álbum de banda sonora (compositor Kei Haneoka): VGMdb y MusicBrainz no tienen ficha de pistas sueltas del OST de temporada 1 con nombres en español o inglés fiables (sólo el single del opening/ending) · ⚠️ No encontré nombres de pista para la música incidental.
...
- Búsqueda específica sobre el recurso cómico de las tarjetas de texto («chapter card», «text card gag», «鬼ごっこ 煽り文字» en japonés): no aparecen vídeos de análisis dedicados sólo a este recurso (los análisis existentes hablan del humor en general, no del rótulo como técnica) · ⚠️ No encontré un vídeo-ensayo centrado en los rótulos.
...
- Nombres de pistas del OST incidental (más allá de opening/ending) en VGMdb/MusicBrainz, en español o inglés — búsqueda: MusicBrainz (vacío, ya lo decía `datos-video.md`), VGMdb (no consultado por API pública disponible desde este servidor).
- Un vídeo-ensayo o análisis dedicado específicamente al recurso de los rótulos/cartelas cómicas de la serie — búsquedas: WebSearch «Kaguya-sama text card gag analysis», «Kaguya-sama narrator captions comedy technique» (inglés).
