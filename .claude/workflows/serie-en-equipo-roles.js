export const meta = {
  name: 'serie-en-equipo-roles',
  description: 'serie-en-equipo sin tipos de agente registrados pero con el modelo de cada rol fijo (recolector, revisor y cierre en Haiku; investigadores en Sonnet; redactor en Opus). Mide las partes con revisor-partes antes de cada redactor, también en modo redactar/seguir. args: {lote: "G", max: 6} o {ids: [...]}, y opcional esfuerzo: {inv, red, aux}',
  whenToUse: 'Cuando la sesión arrancó en una rama sin .claude/agents/ (los agentType no existen) y no se quiere pagar todo con el modelo de la sesión.',
  phases: [
    { title: 'Plan', detail: 'siguiente.py y hermanas.py deciden qué toca y en qué modo' },
    { title: 'Recolectar', detail: 'datos gratis para las series nuevas (Haiku)' },
    { title: 'Investigar', detail: 'un investigador en Sonnet por rol; revisor-partes en Haiku' },
    { title: 'Redactar', detail: 'el redactor en Opus (y un reparo si falta algo)' },
    { title: 'Cerrar', detail: 'revisar.py, subir.sh y el estado en lotes/<L>.md (Haiku)' },
  ],
}
const ROLES = ['imagen', 'video', 'voz', 'texto']
// modelo fijo por rol (el de la línea model: de .claude/agents/<rol>.md)
const MODELO = r => r === 'redactor' ? 'opus' : r.startsWith('investigador-') ? 'sonnet' : 'haiku'
// sin agentType: el agente lee su ficha de rol; el modelo ya va fijado al lanzarlo
const ROL = r => `Eres el subagente «${r}». Antes de nada lee entero /home/user/sintonizando-investigacion/.claude/agents/${r}.md y sigue sus instrucciones como si fueran tu prompt de sistema (su línea model: ya está aplicada). Encargo: `
...
const lanzar = (rol, texto, opts) => agent(ROL(rol) + texto, { ...opts, model: MODELO(rol) })
...
const etapaInv = async (s, it) => {
  ...
  let pend = []
  if (roles.length) {
    const r1 = await parallel(roles.map(rol => () => seguro(() => lanzar(`investigador-${rol}`, `Serie ${it.id}, modo ${modoDe(rol)}.${nota}${hermana(it)}`, { label: `inv:${it.id}:${rol}`, phase: 'Investigar', schema: INV, effort: EF.inv }))))
    ...
    if (pend.length) {
      ...
      const r2 = await parallel(pend.map(rol => () => seguro(() => lanzar(`investigador-${rol}`, `Serie ${it.id}, modo relanzo.${hermana(it)}`, { label: `inv2:${it.id}:${rol}`, phase: 'Investigar', schema: INV, effort: EF.inv }))))
  ...
  if (it.modo !== 'repaso-corto') {
    const rp = await seguro(() => lanzar('revisor-partes', `Serie ${it.id}. Roles: ${ROLES.join(', ')}.`, { label: `revisor-partes:${it.id}`, phase: 'Investigar', schema: FLOJAS, effort: EF.aux }))
    ...
    if (flojas.length) {
      ...
      const r3 = await parallel(flojas.map(f => () => seguro(() => lanzar(`investigador-${f.rol}`, `Serie ${it.id}, modo seguir. Tu parte está floja según revisar_partes.py: ${f.por_que || 'corta'}. Si tu parte no existe o está casi vacía, hazla entera como en modo nueva. Completa lo obligatorio que falte de tus puntos, hasta 50 acciones.${hermana(it)}`, { label: `inv3:${it.id}:${f.rol}`, phase: 'Investigar', schema: INV, effort: EF.inv }))))
...
const etapaRedactor = async (s, it) => {
  ...
  const r = await seguro(() => lanzar('redactor', `Serie ${it.id}, modo ${modo}.${hermana(it)}`, { label: `redactor:${it.id}`, phase: 'Redactar', schema: RED, effort: EF.red }))
...
const cierre = (ultimo) => async (s, it) => {
  ...
  const r = await seguro(() => lanzar('cierre', `Serie ${it.id}, lote ${it.lote}.${ultimo ? ' Es el intento final.' : ''} Resumen y avisos del redactor: «${(s.red || '').replace(/`/g, "'").slice(0, 1500)}»`, { label: `${ultimo ? 'cierre2' : 'cierre'}:${it.id}`, phase: 'Cerrar', schema: CIERRE, effort: EF.aux }))
...
const etapaReparo = async (s, it) => {
  ...
  const r = await seguro(() => lanzar('redactor', `Serie ${it.id}, modo reparo. Falta según revisar.py: «${s.falta}».${hermana(it)}`, { label: `reparo:${it.id}`, phase: 'Redactar', schema: RED, effort: EF.red }))
